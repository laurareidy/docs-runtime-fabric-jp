...
. `azure` ディレクトリの `ARM-template-dev.json` および `ARM-template-prod.json` ファイルの出力を確認します。
...
ARM テンプレートを実行して、Azure アカウントでインフラストラクチャをプロビジョニングします。次の手順では、Azure Portal を介してテンプレートを実行する方法について説明します。
