...
Mule ライセンスキーを Base64 でエンコードするには、`base64 -w0 license.lic` コマンドを使用します。  (OSX では `base64 -b0 license.lic`)。
...
| `RTF_TCP_PROXY` | アウトバウンド TCP 要求を転送する TCP プロキシサーバのホスト名およびポート。 | `1.1.1.2:800`
...
. 以下のコマンドをテキストエディタにコピーして、一重引用符内の各変数の値を入力します。
+
----
RTF_CONTROLLER_IPS='' \
RTF_WORKER_IPS='' \
...
. テキストエディタからコマンドをコピーし、ターミナルに貼り付けて、コントローラ VM でコマンドを実行します。このスクリプトは、各 VM で実行する一連の手順とスクリプトを各 VM の IP アドレスで分類して出力します。
...
* xref:associate-environments.adoc[Runtime Fabric への環境の関連付け]
